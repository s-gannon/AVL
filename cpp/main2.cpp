...
private:
	...
	void search_name(Node* node, string _name, vector<string>& names){
		if (node == nullptr){
			return;
		}
		search_name(node->left, _name, names);
		if (_name.compare(node->name) == 0){
			names.push_back(node->gator_id);
		}
		search_name(node->right, _name, names);
	}
